...
use crate::{
    Color, Framebuffer, Palette, Rect, SpriteSheet, attack::countdown_timer::CountdownTimer,
    draw_sprite, draw_sprite_from_sheet, sprite_blitter,
};
...
impl Default for Screen {
    fn default() -> Self {
        Self {
            pal: Palette::default(),
            _screen: Framebuffer::new(320, 200),
        }
    }
}
...
impl Default for Graphics {
    fn default() -> Self {
        Self {
            pal_2bf: Palette::default(),
            pal_5bf: Palette::default(),
            y_offset: 24,
        }
    }
}
...
impl AttackState {
    pub fn new() -> Self {
        let mut game_state = Self {
            ...
            bg_framebuffer: Framebuffer::new(320, 200),
    ...
    fn load_spritesheet(&mut self) {
        self.sprite_sheet = SpriteSheet::from_possibly_compressed_slice(ATTACK_HSQ).unwrap();

        self.sprite_sheet
            .apply_palette_update(&mut self.gfx.pal_2bf)
            .unwrap();
    ...
    fn draw_background(&mut self) {
        ...
        let y = 0;
        while x < 320 {
            let sprite = self.sprite_sheet.get_sprite(2).unwrap();
            draw_sprite(sprite, x as i16, y + self.gfx.y_offset, fb).unwrap();
            x += sprite.width()
        }
        ...
        let y = 81;
        while x < 320 {
            let sprite = self.sprite_sheet.get_sprite(3).unwrap();
            draw_sprite(sprite, x as i16, y + self.gfx.y_offset, fb).unwrap();
            x += sprite.width();
        }
        ...
        self.bg_framebuffer.copy_from(&self.framebuffer);
    }
    ...
    pub fn draw(&self, screen: &mut Framebuffer, pal: &mut Palette) {
        screen.copy_from(&self.framebuffer);
    ...
    fn sub_10b45(&mut self) {
        ...
        if self.timers.timer4.tick() {
            if self.timers.timer6.tick() {
                let random_val = self.rand();

                self.timers.timer6.set((random_val & 0x7f) as i8);
                self.timers.timer4.set((random_val >> 8) as i16);
            } else {
                let random_val = self.rand();
    ...
    fn sub_10c3b(&mut self) {
        ...
        if self.timers.timer1.tick() {
            if (self.word_1f4b0_rand_bits & 3) == 0 {
                self.timers.timer7.set(11);

                if (self.word_1f4b0_rand_bits & 0x0C) == 0 {
                    self.timers.timer7.set(17);
                }
            }
            ...
            self.timers.timer1.set((masked_random & 0xff) as i8);

            if (masked_random & 0xFF) >= 4 {
                cx |= 0x4000; // Set bit 6 in ch
            }

            self.timers.unk2 = (cx & 0xFF) as u8;
            self.timers.unk3 = ((cx >> 8) & 0xFF) as u8;
        }

        self.timers.timer0.set(8);

        let mut ax = self.timers.unk2 as u16;
        let di = ax;

        let bl = self.timers.unk3;
    ...
    fn sub_10d0d_trigger_sky_palette_flash(&mut self, flag: bool) {
        ...
        if !flag {
            ...
            if self.timers.timer7.get() != 10 {
                // self.gfx_vtable_func_39_transition_palette(al, bx, cx, dl);

                for i in 128..128 + 28 {
                    let divisor = self.timers.timer7.get().max(1) as i16;

                    let c0 = self.gfx.pal_5bf.get(i);
                    let c1 = self.gfx.pal_2bf.get(i);

                    let rgb = c0.lerp(c1, divisor);

                    self.gfx.pal_5bf.set(i, rgb);
                    self.screen.pal.set(i, rgb);
                }
        ...
        apply_palette(&mut self.gfx.pal_5bf, palette);
        ...
        for i in 0..28 {
            ...
            self.gfx.pal_5bf.set(128 + i, color);
            self.screen.pal.set(128 + i, color);
        }
        ...
        for i in 0..28 {
            ...
            self.gfx.pal_2bf.set(128 + i, color);
        }
    }
    ...
    fn sub_1c6ad_particles_update_dirty_rect(&mut self, dirty_rect: &Rect) {
        ...
        for y in rect.y0..rect.y1 {
            for x in rect.x0..rect.x1 {
                let x = x as u16;
                let y = (y + self.gfx.y_offset) as u16;
        ...
        for i in 0..self.particle_count {
            ...
            let clip_rect = Rect {
                x0: 0,
                y0: self.gfx.y_offset,
                x1: 320,
                y1: 200,
            };

            if let Some(sprite) = self.sprite_sheet.get_sprite(particle.sprite_id) {
                sprite_blitter(sprite, &mut self.framebuffer)
                    .at(particle.rect.x0, particle.rect.y0 + self.gfx.y_offset)
                    .clip_rect(clip_rect)
                    .draw()
                    .unwrap();
            };
