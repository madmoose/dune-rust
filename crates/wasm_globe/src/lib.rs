...
impl GlobeRendererInner {
    ...
    fn draw_background(&self, framebuffer: &mut Framebuffer) {
        draw_sprite_from_sheet(&self.fresk, 0, 0, 0, framebuffer).unwrap();
        draw_sprite_from_sheet(&self.fresk, 1, 214, 0, framebuffer).unwrap();
        draw_sprite_from_sheet(&self.fresk, 2, 91, 20, framebuffer).unwrap();
        ...
        for icon in icons {
            draw_sprite_from_sheet(&self.fresk, icon.sprite_index, icon.x, icon.y, framebuffer)
                .unwrap();
        }
    }

    fn draw_head(&self, framebuffer: &mut Framebuffer, head: u8) {
        let head = (16 + head.clamp(0, 10)) as u16;
        draw_sprite_from_sheet(&self.icones, 15, 126, 148, framebuffer).unwrap();
        draw_sprite_from_sheet(&self.icones, head, 150, 137, framebuffer).unwrap();
    }
